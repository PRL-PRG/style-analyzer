# Model report for file:///tmp/top-repos-quality-repos-w91eurp2/quarterly_meeting.git HEAD 33bed47daca3f08c396215415e6ece005970734a
...
| 25 | `  •••start_col ≥ 12<br>	∧ -1.roles in {EXPRESSION} and not in {STRING}<br>	∧ +1.reserved not in {), =, {, }}<br>	∧ +1.roles not in {EXPRESSION}<br>	∧ +1.length ≤ 1<br>	∧ ^1.internal_type not in {ConditionalExpression}<br>	∧ ^1.roles not in {OPERATOR, QUALIFIED}<br>⇒ y = ∅<br>Confidence: 0.997. Support: 5500.` |
...
```json
{"javascript": {"avg_rule_len": 8.296296296296296, "max_conf": 0.9997097849845886, "max_support": 9099, "min_conf": 0.9332298040390015, "min_support": 95, "num_rules": 27}}
```
</details>
