...
make_precision_support_graph <- function (experiment, uniform_scales=TRUE, labels=F) {
  data <- experiment_data[[experiment]]
  plot <- ggplot(data, aes(x=precision, y=support)) +
    geom_point(size=2)
  if (labels) {
  ...
  if (uniform_scales) {
    plot + xlim(min_precision, 1) + ylim(0 - 10000, max_support) # -5000 to fit the stupid label
  } else {
    plot
  }
}
