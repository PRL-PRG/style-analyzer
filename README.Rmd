...
EXPERIMENTS <- c("original_reproduction", "original_10", "random_0_10", "random_1_10")
...
```{r}
make_paper_table <- function(data, latex = FALSE, digits=3) {
  
  data <- data %>% mutate(`training time, min` = round(training_time/60))
  ...
  average <- 
    ...
    summarize(repo = "average", 
              ...
              locs = format(mean(locs), digits=digits),
              ...
              training_time = format(mean(training_time), digits=digits))
  ...
  weighted_average <- 
    ...
    summarize(repo = "weighted average", 
              ...
              locs = format(weighted.mean(locs, support), digits=digits),
              rules = format(weighted.mean(rules, support), digits=digits),
              avg_rule_len = format(weighted.mean(avg_rule_len, support), digits=digits),
              labels = format(weighted.mean(labels, support), digits=digits),
              training_time = format(weighted.mean(training_time, support), digits=digits),
              support = NA)
...
```{r}
make_paper_table(experiment_data$original_reproduction, TRUE)
make_paper_table(experiment_data$original_reproduction)
```
...
```{r}
make_paper_table(experiment_data$random_0_10, TRUE)
make_paper_table(experiment_data$random_0_10)
```

```{r}
make_paper_table(experiment_data$random_1_10, TRUE)
make_paper_table(experiment_data$random_1_10)
```

## Effect of the number of samples in the training set on precision

```{r}
make_precision_support_graph <- function (data) {
...
```{r}
make_precision_support_graph(experiment_data$original_reproduction)
```

```{r}
make_precision_support_graph(experiment_data$original_10)
```

```{r}
make_precision_support_graph(experiment_data$random_0_10)
```

```{r}
make_precision_support_graph(experiment_data$random_1_10)
```
