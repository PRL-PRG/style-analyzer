...
top_starred_projects <- paste0("top_starred_projects_", c(2:6,8:9))
EXPERIMENTS <- c("original_reproduction", "original_10", top_starred_projects, quality_projects)
...
stripped_and_labelled <- lapply(EXPERIMENTS, function(experiment) experiment_data[[experiment]] %>% strip_and_label(experiment))
comparison_data <- do.call(bind_rows, stripped_and_labelled)

support_comparison <- ggplot(comparison_data, aes(x=selection, y=support, color=selection)) + geom_boxplot() + xlab("Selection") + ylab("Samples in training set") + scale_y_continuous(labels=label_number_si()) + theme_bw()+ theme(legend.position="none") 
