...
quality_projects <- paste0("quality_projects_", 10) # 7:9, "_"
EXPERIMENTS <- c("original_reproduction", "original_10", random_projects, random_projects_by_size, "top_starred_projects", quality_projects)
...
comparison_data <- 
  ...
  bind_rows(experiment_data[["top_starred_projects"]] %>% strip_and_label("top_starred_projects")) %>%
  bind_rows(experiment_data[["quality_projects_10"]] %>% mutate(i=1:n()) %>% top_n(20, i) %>% strip_and_label("quality_projects_1")) %>%
  bind_rows(experiment_data[["quality_projects_10"]] %>% mutate(i=1:n()) %>% top_n(-20, i) %>% strip_and_label("quality_projects_2"))

support_comparison <- ggplot(comparison_data, aes(x=selection, y=support, color=selection)) + geom_boxplot() + xlab("Selection") + ylab("Support") + scale_y_continuous(labels=label_number_si()) + theme_bw()+ theme(legend.position="none") 
precision_comparison <- ggplot(comparison_data, aes(x=selection, y=precision, color=selection)) + geom_boxplot() + xlab("Selection") + ylab("Precision") + theme_bw()+ theme(legend.position="none") + scale_y_continuous(limits=c(0.75,1))

grid.arrange(widths=c(1,1), support_comparison, precision_comparison)
```

```{r}
selected_urls <- experiment_data$original_10 %>% pull(url)
data <- all_projects %>% filter(url %in% selected_urls) %>% select(users, stars, size, forks, url, repo) %>% gather("column", "value", users, stars, size, forks, -url, -repo) 
ggplot(data, aes(x = value, y = column, color = "blue", fill = "blue")) +
  geom_density_ridges(stat = "binline", bins = 50, draw_baseline = FALSE, aes(scale = 0.9), alpha = 0.5) + 
  theme_ridges() + 
  theme(legend.position = "bottom") +
  ylab(NULL) + xlab(NULL)

# compare_selection_to_dataset = function(java, python, js, java_sel, python_sel, js_sel, columns, selection_name) {
#   f = function(dataset, selection, columns, color, selection_name) {
#     dataset = dataset %>% select(c(id, columns))
#     dataset = melt(dataset, "id")
#     dataset$kind = "Dataset"
#     selection = selection %>% select(c(id, columns))
#     selection = melt(selection, "id")
#     selection$kind = selection_name
#     x = rbind(dataset, selection)
#     x$kind = factor(x$kind, levels = c("Dataset", selection_name))
#     ggplot(x, aes(x = value, y = variable, color = kind, fill = kind, height = ..ndensity..)) +
#       geom_density_ridges(stat = "binline", bins = 50, draw_baseline = FALSE, aes(scale = 0.9), alpha = 0.5) +
#       theme_ridges() + 
#       theme(legend.position = "bottom") +
#       ylab("") + xlab("") +
...
#   }
#   java = f(java, java_sel, columns, "red", selection_name)
#   python = f(python, python_sel, columns, "blue", selection_name) + 
...
#           axis.ticks.y=element_blank())      
#   java + python + js +
#       plot_layout(ncol =3, nrow = 1, widths = c(6, 6,6), heights = c(6)) +
#       ggtitle(paste0("Comparison of ",selection_name," to entire dataset for Java, Python and JavaScript"))
# }
# 
# g = compare_selection_to_dataset(java, python, js, java_stars, python_stars, js_stars, columns, "Top 1k Stars")
# ggsave("figs/lang_stars_attributes.pdf", plot = g)
# g
```
