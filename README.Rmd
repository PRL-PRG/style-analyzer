...
```bash
cargo run --bin generate_experiment_inputs --release -- --output-path output --dataset-path /mnt/data/codedj-icse/js-selection-merged/ --cache-path /mnt/data/kondziu/codedj-icse/js-selection-merged/
```
...
quality_projects <- paste0("quality_projects_", c(0:3,5:9))
...
short_experiments = PAPER_EXPERIMENTS %>% head(4)

col_index <- 0;
row_index <- 0;
cols <- 2
max_rows <- ceiling(length(short_experiments) / cols)

graphs <- lapply(short_experiments, function(x) {
