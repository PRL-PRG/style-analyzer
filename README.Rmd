...
graphs <- lapply(PAPER_EXPERIMENTS, function(x) make_precision_support_graph(x, use_selection_name = T))
do.call("grid.arrange", c(graphs, ncol=4))
```
...
support_comparison <- ggplot(comparison_data, aes(x=selection_name, y=support, fill=selection)) + 
  geom_boxplot() + 
  xlab("Selection") + ylab("Samples in training set") + 
...
precision_comparison <- ggplot(comparison_data, aes(x=selection_name, y=precision, fill=selection, alpha=!precision_significance)) +
  geom_boxplot() + 
  xlab("Selection") + ylab("Precision") + 
  scale_alpha_discrete(range=c(0.1, 1)) +
  theme_bw() + 
  theme(legend.position="none") + scale_y_continuous(limits=c(0.75,1))
