...
# Collects the number of rules generated by style-analyzer from the report it generates. 
# It just produces one number. Use extract_number_of_rules.sh to get a CSv for the whole dataset.
# Usage: <path/to/test-report.md extract_number_of_rules.awk
...
#
# Each line represents a rule, except weighed avg, macro avg, and micro avg, which are summaries.
# Also except the two rows representing the header and the alignment information.
# To get the number of rules, therefore, count all the rows except those five.
...
# Harvest the rules. They are the second word, they are surrounded by ticks, 
# there is no empty rule.
$2 ~ /^ *`[^`]+` *$/ {    
    # Remove the whitespace padding and the ticks
    rule = $2 # Store to variable so as not to mangle $2
    gsub(/^ *`/, "", rule)
    gsub(/` *$/, "", rule)

    # Store the rule to an associative array: we're using it as a set
    # to make sure that we don't report on duplicate rules
    rules[rule] += 1
}

# Output a summary of the found rules.
END {
    # First, count the rules.
    sum = 0
    for (rule in rules) {
        sum += 1
    }
